...
constexpr std::remove_reference_t<T>&& move_impl(T&& obj) noexcept {
  ...
  static_assert(!std::is_const_v<std::remove_reference_t<T>>, "Cannot move const object");
  static_assert(!std::is_lvalue_reference_v<DeclTypeT>, "Cannot move lvalue, consider copying");
  static_assert(IsIdExpression, "Don't write MOVE(obj.member), write MOVE(obj).member instead");
...
#define move_help_isalpha(c) ((c) >= 'a' && (c) <= 'z' || (c) >= 'A' && (c) <= 'Z')


constexpr bool is_id_expression_impl(const char* const expr) noexcept {
  for (auto str = expr; *str; ++str) {
    if (!move_help_isalpha(*str) && !move_help_isdigit(*str) && *str != '_' && *str != ':') {
      return false;
    }
  }
  return true;
...
#define MOVE(...) move_impl<decltype(__VA_ARGS__), is_id_expression(__VA_ARGS__)>(__VA_ARGS__)
